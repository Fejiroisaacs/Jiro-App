...
Extend the registration flow and `users` table to collect more identity information.

#### Schema changes

New migration: `000011_user_profile.up.sql`
...
- `username` — unique, lowercase, URL-safe handle (e.g. `fejiro`). Used in share links and public profiles.
- `display_name` — free-form name shown in the UI.
- `email_verified` — gated flag; certain features (sharing, notifications) require a verified email.
- `avatar_url` — points to Cloudflare R2 object once file upload is implemented.
...
1. Email + password → account created, `email_verified = false`
2. Verification email sent automatically (see Email Verification below)
3. User prompted to set username and display name on first login (one-time onboarding screen)

#### API changes

| Method | Path | Description |
| ------ | ---- | ----------- |
| PATCH | `/user/me` | Extended to accept `username`, `display_name`, `bio` |
| GET | `/user/:username` | Public profile lookup by username (for share previews) |

---

### Email Verification

Confirm a user owns their email address before enabling sharing and notifications.
...
1. On registration (or manual request), generate a secure random token
2. Hash it (SHA-256) and store in `email_verifications`; send the raw token in the email as a link: `/verify-email?token=<raw>`
3. User clicks link → API looks up the hashed token, checks expiry, sets `users.email_verified = true`, deletes the row
4. Token expires after 24 hours; user can request a resend from the settings page

#### Email verification API endpoints

| Method | Path | Description |
| ------ | ---- | ----------- |
| POST | `/auth/verify-email` | Verify with raw token from email link |
| POST | `/auth/resend-verification` | Re-send verification email (rate limited) |

#### Email transport

- Provider: **Resend** (`resend.com`) — simple REST API, generous free tier (3,000 emails/month), Go SDK available
- Environment variables to add: `RESEND_API_KEY`, `EMAIL_FROM` (e.g. `noreply@yourdomain.com`)
- Resend also covers Chronos notification emails — same integration, no second provider needed

---

### Password Reset

Allow users to reset their password via email when they are locked out.
...
2. API generates a secure random token, hashes it (SHA-256), stores in `password_resets`, emails the raw token as a link: `/reset-password?token=<raw>`
3. User clicks link → frontend shows a "new password" form
4. API validates the hashed token, checks expiry and `used_at`, updates `password_hash`, marks token as used
5. Token expires after 1 hour; only one active reset token per user at a time

#### Password reset API endpoints

| Method | Path | Description |
| ------ | ---- | ----------- |
| POST | `/auth/forgot-password` | Request a reset email (rate limited; always returns 200 to prevent email enumeration) |
| POST | `/auth/reset-password` | Submit new password with raw token |

#### Password reset frontend

- "Forgot password?" link on the login page → `/forgot-password` form (email input)
- `/reset-password?token=...` route → new password form, calls API, redirects to login on success
- Clear error states for expired or already-used tokens

---

#### Email verification frontend

- Banner on dashboard if email is not verified, with "Resend email" button
- `/verify-email` route that calls the API with the token from the URL query param and shows success/error
- Settings page shows verified status with a badge
